...
"@
    ...
    try {
        ...
        if ($runningContainers) {
            ...
            $clickhouseContainers = @($runningContainers | Where-Object { $_ -like "*clickhouse*" })
            ...
            # Get ClickHouse container stats
            if ($clickhouseContainers.Count -gt 0) {
                $statsCmd = "docker stats --no-stream --format ""table {{.Name}}`t{{.CPUPerc}}`t{{.MemUsage}}`t{{.NetIO}}`t{{.BlockIO}}"" " + ($clickhouseContainers -join " ")
                $clickhouseStats = Invoke-Expression $statsCmd 2>$null | Out-String
            }
            ...
            $allStats = "PEERDB CONTAINERS:" + [Environment]::NewLine + $peerdbStats.Trim() +
                       ...
                       [Environment]::NewLine + [Environment]::NewLine + "CLICKHOUSE CONTAINER:" + [Environment]::NewLine + $clickhouseStats.Trim()
        } else {
