...
if docker ps | grep -q clickhouse; then
    ...
    docker exec clickhouse clickhouse-client --query "CREATE DATABASE IF NOT EXISTS target;"
    echo "ClickHouse database 'target' created successfully."
else
    echo "ClickHouse container not found, skipping ClickHouse setup."
fi